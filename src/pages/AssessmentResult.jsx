...
import { Brain, ChevronRight, Download, RotateCcw, TrendingUp, AlertCircle, Clock, Info, ChevronDown, User, Sparkles, RefreshCw } from 'lucide-react'
...
function AiRecommendationCard({ text, loading, error, onRetry }) {
  if (loading) {
    return (
      <div className="card mb-6">
        <div className="flex items-center gap-2 mb-4">
          <Sparkles size={15} className="text-brand-500" />
          <h3 className="font-semibold text-slate-700 text-sm">AI Personalised Recommendation</h3>
          <span className="ml-auto text-xs text-slate-300 animate-pulse">Generating…</span>
        </div>
        {/* Shimmer skeleton */}
        <div className="space-y-2.5 animate-pulse">
          ...
          <div className="h-3 bg-slate-50 rounded-full w-2/3 mt-4" />
          <div className="h-3.5 bg-slate-100 rounded-full w-full" />
  ...
  if (error) {
    return (
      <div className="card mb-6 bg-slate-50 border-slate-100">
        <div className="flex items-center justify-between gap-3">
          ...
          {onRetry && (
            <button onClick={onRetry}
              className="flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-700 px-3 py-1.5 rounded-lg bg-brand-50 hover:bg-brand-100 transition-colors">
  ...
  return (
    <div className="card mb-6 bg-gradient-to-br from-brand-50/60 to-teal-50/40 border-brand-100/60 print:bg-white">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-brand-500 to-teal-500 flex items-center justify-center shrink-0">
          <Sparkles size={12} className="text-white" />
        </div>
        <h3 className="font-semibold text-slate-800 text-sm">AI Personalised Recommendation</h3>
      </div>
      ...
      <p className="text-slate-300 text-xs mt-4 pt-3 border-t border-white/60">
        Generated by AI · Not a medical diagnosis · Always consult your neurologist
      </p>
    </div>
  )
}
...
export default function AssessmentResult() {
  ...
  const [aiText, setAiText]   = useState(null)   // null = not yet tried
  const [aiLoading, setAiLoading] = useState(false)
  const [aiError, setAiError]     = useState(null)
  const aiCalledRef = useRef(false)              // prevent double-fetch in strict mode
  ...
  // Fetch AI recommendation once data is available
  useEffect(() => {
    if (!data || aiCalledRef.current) return
    // If recommendation already saved in DB, use that directly
    if (data.aiRecommendation) {
      setAiText(data.aiRecommendation); return
    }
  ...
  async function fetchAiRecommendation() {
    ...
    try {
      const { scores, overall, type, taskResults } = data
      const band = scoreBand(overall).label
      const text = await generateAiRecommendation({ scores, overall, band, type, taskResults, profile })
      if (text) {
        setAiText(text)
        // Persist to DB so the recommendation doesn't change on revisit
        if (user && id !== 'local') {
          await supabase.from('assessments').update({ ai_recommendation: text }).eq('id', id)
        }
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-brand-50/30 to-white print:bg-white">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <AiRecommendationCard
          ...
          loading={aiLoading}
          error={aiError}
          onRetry={() => { aiCalledRef.current = false; fetchAiRecommendation() }}
        />

        {/* Static fallback summary — shown when AI is unavailable */}
        {!aiLoading && !aiText && (
          <div className="card mb-6">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-2xl p-4 bg-slate-50 border border-slate-100">
                ...
                <p className="text-slate-500 text-sm leading-relaxed">
                  Aim for <strong className="text-slate-700">10–15 minutes of daily practice</strong>. Your next assessment is recommended in <strong className="text-slate-700">2 weeks</strong>.
                </p>
              </div>
            </div>
          </div>
